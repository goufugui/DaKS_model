...
class Attention(nn.Module):
    ...
    def __init__(self,
        ...
        self.query_size = query_size    # 800
        self.memory_size = memory_size or query_size    # 800
        self.hidden_size = hidden_size or query_size    #800
        self.mode = mode
    ...
    def forward(self, query, memory, mask=None):
        ...
        if self.mode == "dot":
            ...
            # attn (batch_size, query_length, memory_length)
            # from (batch_size, query_length, query_size)*(batch_size,memory_size,memory_length)
            attn = torch.bmm(query, memory.transpose(1, 2))  # memory转置 bmm 矩阵乘法 attn每一个元素是X与Ki的点乘积
        ...
        if mask is not None:
            ...
            attn.masked_fill_(mask, -float("inf"))  # mask中为1的位置 在attn中替换"-inf"(负无穷) 即填充知识(个数)的0为负无穷

        # (batch_size, query_length, memory_length)
        weights = self.softmax(attn)    # X dot Ki / sum(X dot Ki) 概率P(Ki|X)
        if self.return_attn_only:
            return weights

        # (batch_size, query_length, memory_size)
        # (batch_size, query_length, memory_length)*(batch_size, memory_length, memory_size)
        weighted_memory = torch.bmm(weights, memory)    # weights*memory 对memory加权
