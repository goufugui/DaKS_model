...
def model_config():
    ...
    data_arg.add_argument("--save_dir", type=str, default="./models_mul_sst_2_travel/")  # 模型训练过程和验证结果保存
    data_arg.add_argument("--with_label", type=str2bool, default=False)
    data_arg.add_argument("--embed_file", type=str, default="./Tencent_AILab_ChineseEmbedding.txt")
    ...
    net_arg.add_argument("--max_vocab_size", type=int, default=None)   # 30000
    ...
    train_arg.add_argument("--lr", type=float, default=0.00005)    # 0.00005
    train_arg.add_argument("--grad_clip", type=float, default=5.0)
    train_arg.add_argument("--dropout", type=float, default=0.3)
    train_arg.add_argument("--num_epochs", type=int, default=60)  # 20
    ...
    train_arg.add_argument("--use_bow", type=str2bool, default=True) # True
    train_arg.add_argument("--use_dssm", type=str2bool, default=False)
    train_arg.add_argument("--use_pg", type=str2bool, default=False)
    train_arg.add_argument("--use_gs", type=str2bool, default=False)  # False
    ...
    misc_arg.add_argument("--log_steps", type=int, default=10)  # 100
    misc_arg.add_argument("--valid_steps", type=int, default=100)  # 200
    misc_arg.add_argument("--batch_size", type=int, default=128)  # 128
    misc_arg.add_argument("--ckpt", type=str)
    #misc_arg.add_argument("--ckpt", type=str, default="models_person_mul_sst_2/best")
...
def main():
    ...
    # Data definition
    corpus = KnowledgeCorpus(data_dir=config.data_dir, data_prefix=config.data_prefix,  # data demo
    ...
    model = KnowledgeSeq2Seq(src_vocab_size=corpus.SRC.vocab_size,
                             ...
                             embed_size=config.embed_size, hidden_size=config.hidden_size,  # 300 800
                             padding_idx=corpus.padding_idx,
                             num_layers=config.num_layers, bidirectional=config.bidirectional,  # 1 true
                             attn_mode=config.attn, with_bridge=config.with_bridge,     # dot true
                             tie_embedding=config.tie_embedding, dropout=config.dropout,    # true 0.3
                             use_gpu=config.use_gpu, 
                             use_bow=config.use_bow, use_dssm=config.use_dssm,  # true false
                             use_pg=config.use_pg, use_gs=config.use_gs,    # false
                             pretrain_epoch=config.pretrain_epoch,  # 5
                             use_posterior=config.use_posterior,    # true
                             weight_control=config.weight_control,  # false
                             concat=config.decode_concat)           # false
    ...
    # Testing
        ...
        print(model)
        try:
          model.load('./SCGD/models_mul_sst_2_travel/best.model')  # models/best.model
        except:
          print("error!")
        print("Testing ...")
        ...
        optimizer = getattr(torch.optim, config.optimizer)(
            model.parameters(), lr=config.lr)   # optim.Adam()
        # Learning rate scheduler
        if config.lr_decay is not None and 0 < config.lr_decay < 1.0:
            lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer=optimizer,
                            factor=config.lr_decay, patience=5, verbose=True, min_lr=1e-5)
        else:
            lr_scheduler = None
        ...
        # Save config
        params_file = os.path.join(config.save_dir, "params.json")  # 路径拼接
